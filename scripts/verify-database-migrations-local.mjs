...
try {
  ...
  assert.deepEqual(installed.changed, [1, 2, 3, 4, 5, 6]);
  ...
  assert.deepEqual(rolledBack.changed, [6]);
  await withClient(names[0], async (sql) => {
  ...
  assert.deepEqual(reapplied.changed, [4, 5, 6]);
  ...
  assert.deepEqual(upgraded.changed, [4, 5, 6]);
  await withClient(names[1], async (sql) => {
    const [{ count }] =
  ...
  try {
    ...
    await writeFile(
      join(temporaryMigrations, "0007_forced_failure.up.sql"),
      "CREATE TABLE must_rollback (id integer); SELECT 1 / 0;",
    );
    await writeFile(
      join(temporaryMigrations, "0007_forced_failure.down.sql"),
