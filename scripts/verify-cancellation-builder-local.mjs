...
if (!minerContainer) {
  throw new Error("CKB_MINER_CONTAINER is required for the stateful cancellation proof");
}
...
const cancellation = await buildCancellation({
  deployment,
  resolver,
  jobOutPoint,
  ownerLock,
...
const cancellationChangeCapacity = creationChangeCapacity - transactionFee;
const completedCancellation = {
  ...cancellation.transaction,
  inputs: [
    ...cancellation.transaction.inputs,
    {
      since: "0x0",
  ...
  outputs: [
    ...cancellation.transaction.outputs,
    { capacity: `0x${cancellationChangeCapacity.toString(16)}`, lock: ownerLock, type: null },
  ],
  outputsData: [...cancellation.transaction.outputsData, "0x"],
};
...
assertCancellationCompletion(cancellation, signedCancellation);
const dryRun = await rpc("dry_run_transaction", [toRpcTransaction(signedCancellation)]);
assert.ok(BigInt(dryRun.cycles) > 0n, "the cancellation must execute contract cycles");
const cancellationHash = await commit(signedCancellation);
assert.equal(await resolver.resolve(jobOutPoint), null, "the cancelled Job Cell must be spent");
await assert.rejects(
  buildCancellation({ deployment, resolver, jobOutPoint, ownerLock }),
  (error) => error instanceof CancellationBuildError && error.code === "STALE_OUTPOINT",
);
console.log(
  `Cancellation builder committed ${cancellationHash} after ${dryRun.cycles} dry-run cycles; ${jobOutPoint.txHash}:0x0 is stale`,
);
